...
const LOCALES = {
  en: {
    ...
    lens: {
      default: "Default",
      axisMetaFrom: (n, v) => `${n} · ${v}+`,
      axisStart: (v, n) => `Axis starts at ${v} — ${n} earlier version${n===1?"":"s"} have no sample on this axis`,
      pinned: "pinned sample",
      pinnedTitle: "The axis sample switches from the default capture to the pinned variant here; the only difference is the model ID spelling (alias vs dated)",
      handoverTitle: "Model generation handover — this diff mixes model and version changes",
    },
    ...
    subscribe: {
      nav: "Subscribe",
      title: "Follow new versions",
      lead: "Every new Claude Code version lands here as an Atom entry — the request delta plus the official changelog. Point your reader at either feed:",
      feedEn: "Atom feed (English)",
      feedZh: "Atom feed (中文)",
      openIn: "Quick add:",
      autodiscover: "Or paste https://api-requests.cc into your reader — the feed is auto-discovered.",
      github: "Prefer GitHub? Watching the repository follows the same updates.",
    },
    anatomy: {
      nav: "Anatomy",
    ...
    delta: { tool:"tool", tools:"tools", modified:"modified", chars:"chars", beta:"beta", betas:"betas", maxTokens:"max_tokens", modelChanged:"model changed", noChange:"No payload change", first:"First captured version", context:"context", systemBlocks:"system blocks", reminderBlocks:"context blocks", section:"section", sections:"sections",
      reminderNames:{ "claude-md":"CLAUDE.md", "auto-memory":"memory", "user-email":"user email", "current-date":"date", "git-status":"git status", "directory-structure":"dir tree", "session-start-hook":"session hook", "available-skills":"skills", "deferred-tools":"deferred tools", "instruction-reminders":"instruction reminder", "ultracode-status":"ultracode", "attribution":"git attribution", "environment":"environment", "other":"reminder" } },
  ...
  zh: {
    ...
    lens: {
      default: "默认",
      axisMetaFrom: (n, v) => `${n} · ${v}+`,
      axisStart: (v, n) => `此轴始于 ${v} —— 更早的 ${n} 个版本无此模型样本`,
      pinned: "钉定样本",
      pinnedTitle: "轴样本自此由默认采集切换为钉定变体，唯一差异是模型 ID 写法（别名 vs 带日期）",
      handoverTitle: "模型世代交接——此行差异同时包含模型与版本两种变化",
    },
    ...
    subscribe: {
      nav: "订阅",
      title: "追踪新版本",
      lead: "每个新版本入库都会生成一条 Atom 条目——请求变化加官方 changelog。把任一 feed 加入你的阅读器：",
      feedEn: "Atom feed（英文）",
      feedZh: "Atom feed（中文）",
      openIn: "快捷添加：",
      autodiscover: "或直接把 https://api-requests.cc 粘贴进阅读器——feed 会被自动发现。",
      github: "习惯 GitHub？Watch 仓库即可跟踪同样的更新。",
    },
    anatomy: {
      nav: "结构",
      title: "请求结构解析",
    ...
    delta: { tool:"个工具", tools:"个工具", modified:"个修改", chars:"字符", beta:"个 beta", betas:"个 beta", maxTokens:"最大 token", modelChanged:"模型已更改", noChange:"载荷无变化", first:"首个捕获版本", context:"上下文", systemBlocks:"system 块", reminderBlocks:"上下文块", section:"个小节", sections:"个小节",
      reminderNames:{ "claude-md":"CLAUDE.md", "auto-memory":"记忆", "user-email":"用户邮箱", "current-date":"日期", "git-status":"git 状态", "directory-structure":"目录树", "session-start-hook":"会话钩子", "available-skills":"技能", "deferred-tools":"延迟工具", "instruction-reminders":"指令提醒", "ultracode-status":"ultracode", "attribution":"git 署名", "environment":"环境", "other":"提醒" } },
    msgStructure: "消息结构",
    captures: "捕获",
