...
/* ---- DeltaSummary ---- */
const DELTA_EN = { tool:"tool", tools:"tools", modified:"modified", chars:"chars", beta:"beta", betas:"betas", maxTokens:"max_tokens", modelChanged:"model changed", noChange:"No payload change", first:"First captured version", context:"context", systemBlocks:"system blocks", reminderBlocks:"context blocks", section:"section", sections:"sections", reminderNames:{} };
function DeltaSummary({ delta, labels=DELTA_EN, tone="inline", style }) {
  ...
  const remAdd=delta.reminders_added||[], remRem=delta.reminders_removed||[], remMov=delta.reminders_moved||[];
  ...
  const secAdd=(delta.system_sections_added||[]).length, secRem=(delta.system_sections_removed||[]).length;
  if(secAdd) parts.push(<Badge key="ssa" tone="add" mono>+{secAdd} {pl(secAdd,L.section,L.sections)}</Badge>);
  if(secRem) parts.push(<Badge key="ssr" tone="del" mono>−{secRem} {pl(secRem,L.section,L.sections)}</Badge>);
